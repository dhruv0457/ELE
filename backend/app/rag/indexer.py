...
class RAGIndexer:
    def __init__(self):
        self.index_path = Path(os.path.expanduser(settings.memory.long_term.index_path))
    ...
    async def index_file(self, file_path: str, project: str = None):
        ...
        await self.index_text(content, tags=tags, source="file", project=project)
    ...
    async def _add_chunk(self, text: str, tags: List[str], source: str, project: str):
        ...
        if self.bm25:
            # Rebuild BM25 (simple approach, could be optimized)
            self.bm25 = BM25Okapi(self.bm25_corpus)
